...
- `DetectionResult`：检测结果（`agent/state.py`）

## 核心组件

- **Router（规划器）**：从快速特征与 Header 认证给出 `FAST/STANDARD/DEEP` 路径与工具计划（`agent/router.py`）
- **Orchestrator（编排器）**：按计划执行工具、处理上下文升级、调用 Policy 输出最终结果（`agent/orchestrator.py`）
- **Tools（证据源）**：将原始输入转为结构化证据（`tools/`）
- **Policy（裁决层）**：硬规则 + 融合评分 → verdict（`agent/policy.py`、`scoring/`）
- **Reporting**：人类可读报告与机器可读 JSON（`agent/report.py`、`agent/cli.py`）
- **Audit**：JSONL 录制与回放（`agent/recorder.py`、`agent/player.py`）
...
路径选择与工具集合在 `configs/default.yaml` 中配置（对应 `agent/config.py`）。
