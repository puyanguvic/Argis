...
- 位置：`tools/url_analyzer.py`
- 输入：URL 列表（可由 `tools/url_utils.extract_urls()` 从正文抽取）
...
- 位置：`tools/attachment_analyzer.py`
- 输入：`attachments` 元信息
...
建议通过 `tools/tool_registry.py` 或在 `agent/orchestrator.py` 的 tool map 处替换实现。
