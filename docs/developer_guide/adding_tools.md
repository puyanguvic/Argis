...
- 在 `tools/` 新增模块（例如 `tools/whois_lookup.py`）
...
- `agent/orchestrator.py` → `_build_tool_map()`
- `agent/orchestrator.py` → `_assign_observation()`

同时更新：

- `configs/default.yaml`：把工具名加入 profile 的 tools 列表

## 4) 评分与报告联动

- 在 `scoring/fusion.py` 增加新因子并赋权
- 在 `agent/report.py` 增加新的 EvidenceLine（可选）
