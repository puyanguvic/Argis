...
def _format_compact_result(final: dict[str, Any], runtime: dict[str, Any]) -> str:
    ...
    skillpack_names = _extract_skillpack_names(runtime)
    skillpacks_line = (
        f"Loaded Skillpacks: {len(skillpack_names)} ({', '.join(skillpack_names[:5])})"
        if skillpack_names
        else "Loaded Skillpacks: 0"
    )
...
def _extract_skillpack_names(runtime: dict[str, Any]) -> list[str]:
    raw = runtime.get("installed_skillpacks", [])
    names: list[str] = []
...
def _format_skillpacks_hint(runtime: dict[str, Any]) -> str:
    names = _extract_skillpack_names(runtime)
    root = str(runtime.get("skillpacks_dir", "")).strip()
    if not names:
        if root:
            return f"Loaded Skillpacks: 0 (dir={root})"
        return "Loaded Skillpacks: 0"
    preview = ", ".join(names[:6])
    if len(names) > 6:
        preview += f", ... (+{len(names) - 6} more)"
    if root:
        return f"Loaded Skillpacks: {len(names)} (dir={root})\n{preview}"
    return f"Loaded Skillpacks: {len(names)}\n{preview}"
...
def _reload_provider_state(provider_mode: str):
    ...
    return (
        ...
        _format_skillpacks_hint(runtime),
...
def build() -> gr.Blocks:
    ...
    with gr.Blocks(
        ...
        with gr.Column(elem_classes=["app-shell"]):
            ...
            with gr.Row(elem_classes=["top-grid"]):
                with gr.Column(scale=4, elem_classes=["panel-card"]):
                    ...
                    skillpacks_hint = gr.Markdown(_format_skillpacks_hint(runtime), elem_classes=["status-box"])
        ...
        provider_mode.change(
            ...
            outputs=[model, runtime_hint, skillpacks_hint, backend_status],
        )
